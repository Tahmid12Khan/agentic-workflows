...
- **Workflow-DSL exception**: `lib/review-workflow.mjs` is NOT a pipeline-step module — it has no shebang/`main` and uses the harness Workflow globals (`agent`/`pipeline`/`phase`/`args`). It cannot be `import`ed or run with `node`; it inlines the pure helpers whose canonical, tested copies live in `lib/review-orchestration.mjs` (orchestration helpers), `lib/verify.mjs` (the verification policy — `selectForVerification`/`resolveVerification`/`partition` plus the cheap→strong escalation decision `firstPassModel`/`shouldEscalate`, run inline instead of via an executor agent; `verifyPolicy`/`cleanVerify` are deliberately NOT inlined — config is resolved once in `plan.mjs` and the sandbox consumes the resolved camelCase `plan.verify` as-is), and `lib/trim-diff.mjs` (`normPath` only — the finding-path normalizer that `inDiffScope` keys on). Keep the inlined copies in sync with their canonical sources. **Args-by-reference**: the diff is NOT inlined into the sandbox — `build-args.mjs` passes `args.diffPath` (+ `args.contextPackPath`) and the reviewer/verifier agents `Read` the diff file themselves, focusing on their file list. So the sandbox no longer inlines `filterDiff`/`stripNoise`/`sectionPath`/`buildDiffIndex`; the only diff-derived datum it consumes, the off-diff `args.diffIndex`, is precomputed by `build-args.mjs` via `trim-diff.mjs`'s `buildDiffIndex`.
