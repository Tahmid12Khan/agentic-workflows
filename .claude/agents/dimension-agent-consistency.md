...
**Out of scope — do NOT flag these as drift.** The pipeline agents deliberately use their own JSON shapes and are not dimension reviewers: `intent-harvester`, `intent-grouper`, `triage-classifier`, `business-logic-analyzer`, `finding-verifier`, `taint-verifier`, `completeness-critic`, `review-synthesizer`, `pr-comment-author`.
