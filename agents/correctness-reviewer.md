...
Rules:
- Only ASSERT a finding at confidence >= 80. If you genuinely cannot decide, emit it with `"uncertain": true` and your best confidence — the orchestrator runs a batched adversarial verification pass (findings grouped by lens+file into a capped set of opus verifiers, plus a reverify false-negative guard) rather than dropping it silently.
