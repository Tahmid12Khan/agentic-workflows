...
- On success, set `checkout = { baseRef, headRef, sha, baseCommit, headCommit }` (for the report — `baseCommit`/`headCommit` carry each side's branch, sha, subject, date and an `origin` block when the reviewed ref diverged from `<remote>/<branch>`) and `ORIGINAL_REF = <originalRef>` (to restore in step 5). HEAD is now detached on the head; **steps 3+ run in the main repo** (no separate cwd) and `--base <baseRef>` makes the diff `baseRef..HEAD`.
