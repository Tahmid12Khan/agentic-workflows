...
- **Cheap false-negative safety net** — near-zero-cost guards against silent misses: an **x1 completeness screen on every workflow tier** (low/standard/high — sonnet, coverage-metadata only, no diff) flags dimension/criterion gaps and re-dispatches a per-tier-capped set of **sonnet** reviewers (**low 0** / **standard 1** / **high 2**; critical runs the fuller opus critic instead); a **cross-file consequence check** asks the correctness reviewer whether each in-repo caller of a changed export still holds; a deterministic **bug-history prior** (recent `fix`/`revert` commits touching the changed files, zero model cost) tells intent + correctness which files deserve extra scrutiny; and an opt-in **test-execution signal** (`--run-tests`) feeds real pass/fail to the test reviewer.
- **Scales to large diffs** — shards a big change into coherent review units; no nested-agent sprawl. A fan-out ceiling (`large_diff.max_review_aspects`, default 40) bounds the total reviewer count on many-dimension diffs so the agent count (and transient-overload exposure) stays capped without dropping any dimension. A file-count ceiling (`large_diff.max_review_files`, default 200) bounds the review payload itself: the orchestrator emits `args.json` verbatim into the Workflow call, so on a many-hundred-file PR that generation can drop mid-response. The payload is kept small by passing everything bulky **by reference** — the diff, the context pack, the per-file diff slices, the concatenated bundle parts reviewers actually `Read`, and the shard file lists all travel as paths to files the agents `Read` themselves, leaving ~55 bytes per changed file (~11 KB on a 69-file PR). Past the ceiling, only the highest-risk files (on a risk path first, then largest churn) are reviewed and the report carries a **WARN** that the rest were not (raise the limit or split the PR).
- **Funnels a mega-PR instead of dropping its tail** — past a much higher `mega_pr.threshold` (default 250 files — deliberately above `max_review_files`, so this never fires on a change today's cap already covers), `lib/funnel.mjs` replaces the blunt cap's silent tail-drop with a two-stage funnel: every risk-path file stays **hot** (fully reviewed); the rest are grouped by (top-level directory, extension), and a group of at least `mega_pr.cluster_min` files (default 8) whose per-file LOC churn all sits within `mega_pr.churn_tolerance` (default ±30%) of the group's median is treated as a **mechanical cluster** — a cheap proxy for "N near-identical boilerplate changes," not a diff-similarity proof — and deterministically **sampled**: `max(mega_pr.cluster_min_sample, ⌈cluster size × mega_pr.sample_rate⌉)` files (defaults 3 / 15%), always including the cluster's single highest-churn file. Nothing vanishes without a trace — the report's **WARN** names every cluster (label, total sampled, total skipped) alongside the existing file-cap WARN. Below the threshold this is a complete no-op; `selectReviewFiles`'s cap behaves exactly as before.
...
4. **Workflow fan-out** (`lib/review-workflow.mjs`) — the Workflow owns intent → review → verify → synthesize, then returns the assembled report **payload** (it does not render — the sandbox can't write files):
   - **Intent** — `triage-classifier` (sonnet, skipped on the trivial tier) first sanity-checks the tier (raises it for the human when blast radius warrants, and **adds dimensions the rules missed** as real review aspects); then a single `intent-analyzer` pass (the former `intent-harvester` + `business-logic-analyzer`, merged) builds the acceptance-criteria model (stated vs derived + mismatches), the primary-vs-extra intent grouping, **and** the domain/business-logic model (assumptions + open questions) — reasoning in **stages** (criteria/groups before assumptions/questions) so the merge keeps the old producer→consumer barrier. **It Reads the same whole-diff bundle parts D3 uses (`args.allParts`)** in one batch of parallel `Read` calls instead of self-paginating the bare diff — the bundle already excludes lockfile/build-artifact/vendored churn (that filtering happens upstream, before the bundle is built), and once it has read every part it caps any further Read/Grep/Bash lookups at 3; falls back to `Read`ing the raw `args.diffPath` (with the ignore-churn instruction restored) when no bundle was built. Plus the deterministic **bug-history prior** (`history.mjs`: recent `fix`/`revert` commits touching the changed files). Runs at **low+** so even low-tier reviewers get an intent brief.
   - **Review** — `correctness-reviewer` always; the planned specialist agents per dimension; one pass per shard for large diffs. **Args-by-reference + bundle-parts reading (cost lever):** `build-args.mjs` writes a per-file diff slice for each changed file, then concatenates each shard's slices into a handful of **bundle parts** (`bundleParts`, capped under the `Read` limit, `=== FILE: <path> ===` headers marking the boundaries) so a reviewer **`Read`s a few bundle parts in one parallel batch** (`scopeFor`) instead of one slice per file or the whole diff; falls back to the shard's manifest, then to per-file slices (`diffReadFor`), then to the full `args.diffPath`, if a bundle wasn't written (D3/security gets its own whole-diff bundle, `args.allParts`, covering every reviewed file for cross-file taint, falling back to the bare diff only if that bundle is empty); it also gets an **intent brief scaled to what it actually uses** (`briefFor`) — `correctness-reviewer` (D1 intent alignment is its own dimension) and `test-adequacy-reviewer` ("does each criterion have a test") get the full brief: criteria + mismatches + the groups flagged for scrutiny, whose file lists are carried as **basenames** since the reviewer gets every full path it owns from its bundle parts' `=== FILE: <path> ===` headers (or, on the manifest fallback path, from its shard manifest); every other reviewer gets the **one-line intent summary alone**, because its instructions never check the criteria — and a **shared context pack** (`context-pack.mjs`, built once in step 3: the enclosing definitions of changed code, import blocks, in-repo callers of changed exports, a **hop-2 section** — each caller's own enclosing definition SIGNATURE only, one contract level up, capped at 8 per file — and, for changed TS/Python files, a **type-boundary section** (the interface/dataclass definitions referenced on the changed lines, headed `## for: D10,D11` so `api-compat-reviewer`/`type-design-reviewer` know it's for them) — passed by path in `args.contextPackPath`; hop-2 and the type-boundary are the first extras dropped under the pack's byte caps, so the mandatory enclosing-definition body is never truncated) — the shared blocks lead the prompt so they **prompt-cache across every aspect of the same reviewer** (caching is **intra-agent only** — distinct agents have distinct system prompts, so nothing caches across the intent/review/verify boundary; the report's **cache-hit% panel** shows the real per-run rate). **On demand, not upfront (cost lever):** `context-pack.mjs` also writes each file's section to its own fragment file under `args.contextDir`, named with the SAME `sliceName` a file's diff slice uses — a reviewer that already knows a file's slice name knows its fragment name for free, and pulls ONLY the fragment for a file it suspects has a finding, inside its existing lookup budget (never the whole pack up front). Only when the fragment write failed (`contextDir` absent) does a reviewer fall back to the ORIGINAL whole-pack instruction — `Read` the pack FIRST, before the diff. Either way, reviewers make at most ~4 extra lookups — `Read`, `Grep`, or **read-only `Bash`** (`git show/log/blame`, `rg`, `sed -n`; writes are denied by the `PreToolUse` allowlist in `lib/allow-bash.mjs`) — only when the pack/fragment is insufficient for a specific suspected finding. D3/security is the exception: it runs as a **single unsharded pass reading the whole-diff bundle parts** (so cross-file taint survives, instead of re-paying the whole diff once per shard). **Dimension-scoped bundles (cost lever):** three routable dimensions — `test-adequacy-reviewer` (D5 → test files), `data-store-reviewer` (D6 → migration/repository files; excluded whenever D8 is also active, since D8's connection/resource-leak concerns don't live in those files), `api-compat-reviewer` (D10 → contract files, e.g. `.proto`/OpenAPI/GraphQL/controllers) — get a narrower manifest/bundle precomputed per shard, covering only their matching files, instead of the whole shard; an agent mixing a routed and an unrouted dimension, or a shard where nothing matches, keeps the FULL shard (never a partial narrow, never a zero-file dispatch). `correctness-reviewer` (D1/D2/D12) and D3/`vuln-reviewer` are deliberately excluded from routing and always see the full shard/diff — the same two always-on, always-full-scope reviewers plus the completeness-critic's exhaustive-mode gap audits are the plan's named mitigations for this narrower-scope risk. Extra-intent groups get focused scrutiny. Per-reviewer extras ride the packet: the **correctness reviewer** gets a **cross-file consequence** directive (does each listed caller still hold?) plus the **bug-history prior**; the **test-adequacy reviewer** gets the **executed-test signal** when `--run-tests` ran.
   - **Verify (batched, sonnet-first)** — on non-trivial tiers (`plan.runVerify` true), the **unsure** findings — low-confidence, flagged uncertain, or high-severity on a risk path (`selectForVerification`) — are **grouped by (verifier lens, file) into ≤ `maxVerifierAgents` groups** (per tier: 3/5/8) and refuted a group at a time: one `finding-verifier` (or `taint-verifier` for D3 security) per group returns a verdict for every finding it holds — each **`Read`s only the group's per-file diff slices** (`diffReadFor`) **plus that group's on-demand context-pack fragments** (`args.contextDir`, the same per-file fragments reviewers get) with a **generous 7-lookup budget** (up from a reviewer's 4 — a verifier is the safety net, so it isn't starved); the `taint-verifier` keeps the full diff and no fragments, so cross-file taint survives. **Cost lever — sonnet-first:** a first-pass group runs on the cheap model (`verify.model_first`, default `sonnet`); only a group holding a **critical** finding and the taint verifier go straight to `opus`. Confident, non-risk findings are trusted and ship at the ≥80 gate. Then **one extra opus reverify guard** re-examines the refuted/uncertain hot findings for false negatives (bias inverted — uphold unless the refutation clearly holds, so opus adjudicates every kill), so **total verifier agents ≤ N+1**. Every selected finding is verified — the cap bounds agent count, not coverage; an unresolved finding → "needs human". On **every workflow tier** (low/standard/high) a cheap **x1 completeness screen** (sonnet, coverage-metadata only — no diff, so dimension/criterion gaps, not taint) reuses `completeness-critic` and re-dispatches a per-tier-capped set of **sonnet** reviewers (**low 0** / **standard 1** / **high 2**). On **exhaustive** reviews (critical/`--exhaustive`) the full `completeness-critic` (opus, with the diff) instead hunts for what was **missed** (unrun dimension, uncovered criterion, untraced taint) and re-dispatches ≤ 6 targeted reviewers. Either way the new findings re-enter the batched Verify.
...
| Low | small localized logic w/ tests | one reviewer + x1 sonnet completeness screen (0 gap re-dispatch) |
| Standard | normal feature/bugfix | correctness + screens + x1 sonnet completeness screen (≤1 gap) |
| High | shared lib, API contract, perf hot path | full fan-out + simplify + bounded verify + x1 sonnet completeness screen (≤2 gaps) |
...
| D13 docs | docs-comment-reviewer | sonnet |
...
lib/
  ...
  funnel.mjs      mega-PR two-stage funnel — classify hot vs. mechanical-repeat clusters, sample deterministically (pure)
  ...
  build-args.mjs  pre-step outputs (plan/bundle/diff/context/routing/doctrine) → Workflow args, file→file (diff, pack, slices, bundle parts + shard manifests never enter agent context)
  allow-bash.mjs  PreToolUse guard — reviewers get Bash, but only read-only commands (CLI + pure)
...
hooks/      hooks.json — PreToolUse(Bash) → lib/allow-bash.mjs (read-only allowlist for this plugin's agents)
...
- **Advisory by construction** — reviewers have `Bash` so they can pull the context they need, but a `PreToolUse` allowlist (`lib/allow-bash.mjs`) denies every command that could write: non-allowlisted binaries, write git subcommands, redirection, command substitution, `sed -i`, `find -exec`/`-delete`. The guard applies only to this plugin's own agents and stays silent for anything else, so it never affects your own shell.
