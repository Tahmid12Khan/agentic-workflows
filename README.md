...
- **Scales to large diffs** — shards a big change into coherent review units; no nested-agent sprawl. A fan-out ceiling (`large_diff.max_review_aspects`, default 40) bounds the total reviewer count on many-dimension diffs so the agent count (and transient-overload exposure) stays capped without dropping any dimension. A file-count ceiling (`large_diff.max_review_files`, default 200) bounds the review payload itself: the orchestrator emits `args.json` verbatim into the Workflow call, so on a many-hundred-file PR that generation can drop mid-response. The payload is kept small by passing everything bulky **by reference** — the diff, the context pack, the per-file diff slices, and the shard file lists all travel as paths to files the agents `Read` themselves, leaving ~55 bytes per changed file (~11 KB on a 69-file PR). Past the ceiling, only the highest-risk files (on a risk path first, then largest churn) are reviewed and the report carries a **WARN** that the rest were not (raise the limit or split the PR).
...
lib/
  ...
  build-args.mjs  pre-step outputs (plan/bundle/diff/context/routing/doctrine) → Workflow args, file→file (diff, pack, slices + shard manifests never enter agent context)
