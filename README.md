...
- **Done (v0.5.0)** — **verify-unsure-only**: `selectForVerification` gates the verify stage — confident, non-risk findings (≥80 confidence, not on a risk path) ship without a verifier pass, replacing the prior verify-all behaviour. **Inline resolve**: `verifyPolicy`/`resolveVerification`/`partition` inlined in the Workflow DSL — eliminates the executor agent spawned for `node verify.mjs resolve`. **Remove `pr-comment-author` from Workflow** — inline PR comments are posted deterministically by `lib/comments.mjs` from the command; 22 bundled agents (was 23). **Report executor on haiku.**
