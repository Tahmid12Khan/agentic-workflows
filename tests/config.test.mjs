...
test('project config is valid JSON and has a gate', () => {
  const cfg = JSON.parse(readFileSync('.adversarial-code-review/config.json', 'utf8'));
...
test('project config has no keys the schema forbids (additionalProperties:false blocks)', () => {
  const cfg = JSON.parse(readFileSync('.adversarial-code-review/config.json', 'utf8'));
  const schema = JSON.parse(readFileSync('.adversarial-code-review/config.schema.json', 'utf8'));
  const errors = [];
