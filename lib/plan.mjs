...
const out = {
  ...
  // Dimension-scoped bundles (#9) off-switch: an explicit `false` disables the file-narrowing
  // mechanism entirely (every aspect falls back to full scope, as if ROUTED were empty) — the same
  // gate+report precedent as large_diff/mega_pr/fanout, for a mechanism that previously had neither.
  routing: { enabled: config.routing?.enabled ?? true },
