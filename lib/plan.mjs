...
const out = {
  ...
  diffSummary: `${files.length} files, ~${netLoc} LOC, langs: ${signals.languages.join(',') || 'n/a'}, risk: ${signals.riskPaths.join(',') || 'none'}${filesFunneled ? `, funneled: ${filesFunneled.hot} hot + ${filesFunneled.sampled}/${filesFunneled.mechanicalTotal} mechanical sampled` : ''}${filesCapped ? `, reviewing ${filesCapped.reviewed}/${filesCapped.total} (capped)` : ''}${sharded ? `, ${shards.length} shards` : ''}`,
};
