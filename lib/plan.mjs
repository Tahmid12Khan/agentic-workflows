...
// Reads .review/config.json from cwd (optional). Prints a JSON plan to stdout.
...
// --- config ---
let config = {};
if (existsSync('.review/config.json')) {
  try { config = JSON.parse(readFileSync('.review/config.json', 'utf8')); } catch { /* ignore */ }
}
...
const out = {
  ...
  learning: config.learning ?? { enabled: true, store: '.review/learnings.json' },
