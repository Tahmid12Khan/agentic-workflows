...
if (wantIncremental && !forceFull) {
  ...
  // .adverserial-code-review/last-review.json state to read. Falls back to that store otherwise.
  const prevHead = arg('--incremental-from') ?? loadLastReview('.adverserial-code-review/last-review.json')?.head ?? null;
...
const out = {
  ...
  verify: {
    ...
    maxVerifierAgents: vp.maxVerifierAgents,
