...
// which drives shards, sliceIndex, diffIndex — is trimmed.
const maxReviewFiles = ld.max_review_files ?? 200;
const { files: reviewFiles, capped: filesCapped } = selectReviewFiles(files, { max: maxReviewFiles, riskPaths: signals.riskPaths, locByFile });
...
const out = {
  ...
  diffSummary: `${files.length} files, ~${netLoc} LOC, langs: ${signals.languages.join(',') || 'n/a'}, risk: ${signals.riskPaths.join(',') || 'none'}${filesCapped ? `, reviewing ${filesCapped.reviewed}/${filesCapped.total} (capped)` : ''}${sharded ? `, ${shards.length} shards` : ''}`,
};
