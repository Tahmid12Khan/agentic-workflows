...
// dimension reviewer Reads ONLY its files' hunks (args.sliceDir + sliceName) instead of the whole diff —
...
// Pure: the on-disk slice FILENAME for a changed file, derived from the path itself.
//
// Why derived and not mapped: `args` must be emitted VERBATIM by the orchestrator into the
// Workflow tool call, and a path→absolute-slice-path map carried one long entry per reviewed file
// — the largest per-file term in that payload, and the reason a big PR could drift mid-generation
// (see the file-count ceiling in lib/shard.mjs). Deriving the name lets args carry the slice
// DIRECTORY once instead. Both sides must agree on the name with no shared state: build-args.mjs
// writes the file, the Workflow sandbox (no fs, no imports) recomputes the same name to tell an
// agent what to Read. FNV-1a 32-bit over the normalized path, base36 — short, deterministic,
// filesystem-safe at any path depth, and free of Date/random (golden rule 4).
//
// Collisions are ~1 in 200k at the 200-file ceiling, but a collision would silently give two files
// one slice, so build-args detects it and disables slicing for the whole run rather than
// mis-routing a reviewer.
export function sliceName(path) {
