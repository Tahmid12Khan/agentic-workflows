...
// `-c core.quotePath=false` so a non-ASCII path is emitted raw (e.g. `+++ b/café.js`) instead of
// quoted+octal-escaped (`+++ "b/caf\303\251.js"`) — the mangled quoted form never round-trips
// through normPath/buildDiffIndex's path keys, silently dropping the file from diff scope.
export function diffArgs(base, head = 'HEAD') {
  return ['-c', 'core.quotePath=false', 'diff', '--no-color', `${base}..${head}`];
}
