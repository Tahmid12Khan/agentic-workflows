...
export const DEFAULT_VERIFY = {
  ...
  // group refutes all its findings in a single pass (reading only that group's per-file diff slices),
  // so the diff is paid once per group instead of once per finding and agent count stops scaling with
  // finding count. COST LEVER — the groups are SONNET-FIRST: a first-pass group runs on `modelFirst`
  // (sonnet); only a group holding a finding in `escalateDirectSeverity` (critical) and the
  // taint-verifier go straight to `verifyModel` (opus). Then a single extra `verifyModel` (opus)
  // REVERIFY guard re-checks the refuted/uncertain hot findings for false negatives with the bias
  // inverted — so opus adjudicates every costly kill while the bulk refutation stays cheap. Total
  // verifier agents ≤ maxVerifierAgents + 1.
  maxVerifierAgents: 5,       // per-run cap on batched verifier groups (per-tier default via MAX_VERIFIER_AGENTS_BY_TIER)
  verifyModel: 'opus',        // model for a critical-severity group, the taint verifier, and the reverify guard
  // Cheap→strong split for the batched path: modelFirst runs the first-pass groups, verifyModel (opus)
  // is the escalation target for a critical group + the reverify guard. escalateDirectSeverity names the
  // severities that skip the cheap first pass. (The per-FINDING firstPassModel/shouldEscalate helpers
  // below are the LEGACY per-finding form — kept for config back-compat + unit tests; the workflow uses
  // the simpler group-level severity check inline, not those functions.)
  modelFirst: 'sonnet',
  modelEscalate: 'opus',
...
// - a CRITICAL-severity finding, ALWAYS (WS6: cheap + highest cost-of-miss; also ensures off-diff
//   criticals are verified so the "Pre-existing bugs" section only carries upheld defects — selection
//   runs before the diff-scope split, so this is where an off-diff critical gets caught), OR
