...
// rewritten from addyosmani/agent-skills. This module only maps agent/tier -> fragment BASENAMES;
// build-args.mjs resolves the basenames here to absolute paths under <pluginRoot>/agents/doctrine/,
// reads each fragment's text, and inlines it directly into the reviewer's prompt (the fragments are
// small enough that a Read-by-reference round trip isn't worth the extra turn). Keys are AGENT names
// (an aspect covers one agent's whole dim set), not dimensions.
//
...
// resolves its doctrine — the Workflow only reads doctrineText[agent] when that agent actually runs.
