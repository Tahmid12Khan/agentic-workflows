...
// Force the intent model to a structured object (was free-form text → escaped-string blob
...
// statedIntent/derivedIntent/expectedTests/outOfScope stay only on the deep consumers
// (business-logic, completeness-critic, synth). Mirrors agents/intent-harvester.md's documented shape.
...
const { plan, bundle, diff, contextPack, shards, routing, flags, startedAt, prNumber, checkout } = A;
...
const harvester = await agent(
  ...
  { agentType: pluginAgent('intent-harvester'), phase: 'Intent', schema: HARVEST_SCHEMA },
...
phase('Review');
// Compact intent brief for the dimension reviewers: the criteria they trace against, the
// mismatches to confirm, and ONLY the groups flagged for scrutiny. The full prose model
// (statedIntent/derivedIntent/expectedTests/outOfScope/extraIntents) is reserved for the deep
// consumers (business-logic above, completeness-critic + synth below). Falls back to the raw
// value if the harvester returned a string (schema miss) so a reviewer is never starved of intent.
const harvesterBrief = (harvester && typeof harvester === 'object')
  ? { summary: harvester.summary, acceptanceCriteria: harvester.acceptanceCriteria, mismatches: harvester.mismatches, scrutinize: (harvester.groups ?? []).filter((g) => g?.scrutinize) }
  : harvester;
...
const reviewed = await pipeline(
  ...
  // trail so they never poison that prefix. (Same diff-first cache trick the intent/verify passes use.)
  (a) => agent(
    `${packBlock}` +
    `Acceptance criteria, mismatches + flagged intent groups (groups marked scrutinize warrant extra attention): ${JSON.stringify(harvesterBrief)}. ` +
  ...
  if (gaps.length) {
    ...
    const reDispatched = await parallel(gaps.map((g) => () =>
      agent(
        ...
        `Acceptance criteria: ${JSON.stringify(harvesterBrief)}. Project rules: ${JSON.stringify(plan.projectRules)}. Diff:\n${diff}`,
