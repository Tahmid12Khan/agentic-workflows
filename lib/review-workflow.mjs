export const meta = {
  name: 'acr-review',
  description: 'Adversarial code review fan-out: intent, dimension review, batched Verify (≤N opus groups + 1 reverify guard), synthesize, render.',
  phases: [
    { title: 'Intent' },
    { title: 'Review' },
    { title: 'Verify' },
...
// the FULL diff (cross-file source→sink); a generic group is file-scoped (diff paid once for the
// group's files). Diff-first ordering keeps that prefix cacheable if the same group scope recurs. A
// failed group degrades every finding in it to 'uncertain' (never crashes the run).
const spawnBatchVerifier = (group, verdictByKey) => {
  const payload = group.findings.map(verifyPayload);
  const dForGroup = group.verifier === 'taint-verifier' ? diff : filterDiff(diff, group.files);
  return agent(
    ...
    { agentType: pluginAgent(group.verifier), model: verifyModel, label: `verify:${group.verifier}:${group.files[0] ?? 'multi'}×${payload.length}`, phase: 'Verify', schema: VERDICTS_SCHEMA },
...
// Batched verification over the full finding set: ≤ maxVerifierAgents opus groups + 1 opus reverify guard.
phase('Verify');
