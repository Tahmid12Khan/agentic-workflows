...
function routedFiles(dims, files) {
  // TEST_FILE also covers pytest's primary `test_*.py` convention (not just `*_test.py`/a `tests/`
  // dir), a bare `_spec.<ext>` suffix with no `spec/` directory required (Ruby et al.), and the
  // C#/Java/Kotlin `Test`/`Tests` suffix with no separator before it — a PARTIAL match (some files
  // caught, most missed) is the dangerous case here, not the all-or-nothing empty-match fallback.
  const TEST_FILE = /(^|\/)(tests?|__tests__|spec)(\/|$)|\.(test|spec)\.[jt]sx?$|_test\.[a-z]+$|(^|\/)test_[^/]*\.py$|_spec\.[a-z]+$|Tests?\.(cs|java|kt)$/i;
  const DATA_FILE = /\.sql$|migrat|flyway|liquibase|alembic|knex|prisma[\\/]migrations|(^|\/)(repository|dao)(\/|\.)|Repository\.(java|ts|py)$/i;
  // CONTRACT_FILE also covers a `Controller`-suffixed file with no `controller/` directory required
  // (e.g. `UserController.java` living under `src/api/`, not a `controller/` dir).
  const CONTRACT_FILE = /\.proto$|openapi|swagger|graphql|(^|\/)controller|Controller\.[a-z]+$/i;
  // D8 ("connections & resources" — pool sizing, connection leaks, closeable lifecycle, HTTP
  // keep-alive) is deliberately NOT routed here: none of that lives in .sql/migration/repository
  // files, so it must never ride on D6's DATA_FILE test. An agent whose dims include D8 therefore
  // always hits the "any dim unrouted → full scope" rule below, never a partial (and wrong) narrow.
  const ROUTED = { D5: TEST_FILE, D6: DATA_FILE, D10: CONTRACT_FILE };
...
function expandAspects(dimensionAgents = {}, shards = [], { unsharded = [], allManifest = null, allParts = null, routingEnabled = true } = {}) {
  ...
  for (const a of order) {
    ...
    // routingEnabled (config.routing.enabled, default true): an off-switch for the whole mechanism —
    // when explicitly false, every aspect here is left exactly as `add()` set it (full shard scope),
    // as if ROUTED were empty, regardless of whether a PRIMARY routed[agent] entry exists on disk.
...
function parseWorkflowArgs(raw) {
  // Both constants are LOCAL, not module-level, so this whole function extracts as one
  // self-contained unit for the SYNCED_FUNCTIONS sync test (same reason as routedFiles).
  // `\x7d` IS the closing-brace character, escaped on purpose: the SYNCED_FUNCTIONS sync test
  // extracts this body by naive brace counting, so a literal closing brace anywhere in here (regex,
  // string, or even a comment) ends the extraction early and silently compares only the prefix.
  const TRAILING_NOISE = /[\s\x7d\]]$/;
  const MAX_TRIM = 8;
  if (raw == null) return { args: {}, repaired: false, note: null };
  if (typeof raw !== 'string') return { args: raw, repaired: false, note: null };
  let firstError;
  try {
    return { args: JSON.parse(raw), repaired: false, note: null };
  } catch (e) {
    firstError = e;
  }
  let cut = raw.replace(/\s+$/, '');
  for (let i = 0; i < MAX_TRIM && TRAILING_NOISE.test(cut); i++) {
    cut = cut.slice(0, -1).replace(/\s+$/, '');
    let args;
    try { args = JSON.parse(cut); } catch { continue; }
    const discarded = raw.slice(cut.length).trim();
    return {
      args,
      repaired: true,
      note: `args had ${discarded.length} surplus trailing character(s) (${JSON.stringify(discarded)}) — transcription drift over the nested tail; parsed the valid prefix.`,
    };
  }
  throw new Error(`args JSON malformed (${firstError.message}) — likely tool-call transcription drift over a large payload; re-emit the Workflow call with the exact contents of args.json.`);
}
const parsedArgs = parseWorkflowArgs(args);
const A = parsedArgs.args;
const { plan, bundle, diffPath, contextPackPath, diffIndex, historyPath, testSignal, shards, routing, flags, startedAt, prNumber, checkout, diffRanges, sliceDir, contextDir, doctrineText, contextPackStats, knownFalsePositives, allManifest, allParts, buildNotes } = A;
...
// of changed code, import blocks, and in-repo callers of changed exports. ARGS-BY-REFERENCE, with
// two delivery modes depending on what build-args.mjs found on disk (checked in this order):
//  - contextDir (ON-DEMAND FRAGMENTS, the cost lever this unlocks): context-pack.mjs wrote one
//    fragment per reviewed file under contextDir, named with the SAME sliceName derivation as that
//    file's diff slice (lib/trim-diff.mjs) — so a reviewer that already knows its own files' slice
//    names (from its manifest/bundle) needs no new lookup logic to find the matching fragment. It
//    is told to pull ONLY the fragment for a file it suspects has a finding, inside its EXISTING
//    Read/Bash lookup budget — never the whole pack up front.
//  - contextPackPath alone (fragments unavailable): the ORIGINAL whole-pack behavior — Read it
//    FIRST, before the diff.
// Both null (pre-step skipped/failed) → packBlock is '', so a reviewer falls back to its own
// Read/Grep, same as before this feature existed. The instruction is byte-identical across every
// aspect of the same reviewer agent, so it LEADS the packet and prompt-caches across them. The
// caller section rides in the pack; the correctness reviewer is its primary consumer (S6.2).
const packBlock = contextDir
  ? `Per-file CONTEXT FRAGMENTS (enclosing definitions of the changed code, import blocks, and in-repo callers of the changed exports — one fragment per file) are on disk at ${contextDir}/<slice-name>, named with the SAME slice-name convention as your diff slice (lib/trim-diff.mjs sliceName) — if you already know a file's diff-slice name, you already know its context-fragment name. Do NOT read them all upfront: pull ONLY the fragment for a file you suspect has a finding, before finalizing that finding, using your existing Read/Bash lookup budget below (no separate budget for this). If a fragment is absent, that file had no extractable context — fall back to your own Read/Grep rather than retrying.\n\n`
  : contextPackPath
...
// Same by-reference lookup as slicesFor, but for the per-file CONTEXT fragments Task 1 wrote under
// contextDir (one per reviewed file, named via the SAME sliceName as its diff slice — see packBlock
// above). Used to give a batched VERIFIER group its files' context fragments, which today it gets
// zero of (a verifier only ever saw the diff slice + the finding payload). '' / no contextDir → [].
const contextFragmentsFor = (files = []) => (contextDir
  ? [...new Set((files ?? []).map(normPath).map(sliceName).filter((n) => reviewedSlices.has(n)).map((n) => `${contextDir}/${n}`))]
  : []);
// COST LEVER (bundle parts): every whole/shard-scope consumer of a pre-concatenated bundle (D3, a
// sharded dimension reviewer, intent) gets the same "Read them all in one batch, check for
// truncation" instruction; only the TRAILING clause (what to do with that coverage) differs per
// caller, so it is factored out here instead of drifting across three near-identical literals.
// build-args.mjs's bundleParts() sizes each part to fit under the Read tool's real per-call limit,
// but a dense enough file can still trip it, so every consumer is told to check for a truncation
// notice on any Read and page through with Read(offset=...) rather than trust its own completeness.
const bundleReadInstruction = (parts, trailing) =>
  `${parts.length} bundle part(s) cover this scope: ${parts.join(', ')} — Read ALL of them now, `
  ...
  + `where one file's hunks end and the next begins. Parts are sized to fit under the Read tool's `
  + `per-call limit, but check every Read's result for a truncation notice — if you see one, page `
  + `through the rest of that part with Read(offset=...) before treating it as fully read.\n\n`
  + `${trailing}`;
...
const scopeFor = (a, dimList) => {
  ...
  if (isD3) {
    ...
    if (allBundleParts.length) {
      return bundleReadInstruction(allBundleParts, `Review EVERY changed file across those parts for dimension(s) ${dimList}.`);
    }
    return `${diffRead}\n\nReview EVERY changed file in that diff for dimension(s) ${dimList}.`;
  }
  const parts = a.parts ?? [];
  if (parts.length) {
    return bundleReadInstruction(parts, `Review ONLY the files covered by those parts, for dimension(s) ${dimList}: ${a.count ?? 0} file(s).`);
  }
  if (a.manifest) {
...
// A repaired args payload still produced a full review, but the user must see that the orchestrator's
// transcription drifted — silent recovery would hide a regression in what reaches the sandbox.
if (parsedArgs.note) notes.push(parsedArgs.note);
const agentRuns = {};
...
// COST LEVER (bundle parts, Task 4): intent-analyzer runs ONCE globally and previously read the
// bare, unsliced diff — on a measured production run it self-paginated via repeated
// Read(offset=...) calls and ran out of budget 44% of the way through a large diff. Same fallback
// shape as D3's scopeFor branch (Task 2): prefer allParts (pre-bundled, and already noise-free —
// lockfiles etc. are excluded from reviewFiles upstream in plan.mjs, before parts are built), and
// fall back to the bare diffRead (with the lockfile-churn caveat restored, since the raw diff at
// diffPath is NOT pre-filtered) when no bundle was built.
const intentParts = allParts ?? [];
// Fix 10: the budget below caps FURTHER calls beyond the bundle-part Reads, but the SAME prompt may
// also carry a PRIOR BUG HISTORY read (historyPrior, appended below) — that read plausibly consumes
// one of the budgeted calls without being separately accounted for. Bumped 3 → 4 to match the
// dimension reviewers' existing "4 additional lookups" convention and to leave room for it.
const intentDiffRead = intentParts.length
  ? bundleReadInstruction(intentParts, `Reading all parts gives you complete diff coverage of every changed file — cap any FURTHER Read/Grep/Bash calls at 4 (this budget also covers a PRIOR BUG HISTORY read below, if present), reserved for a specific sibling file you need for context.`)
  : `${diffRead} (ignore lockfile/build-artifact/vendored churn — focus on the source/test/config changes).`;
const intentP = agent(
  `${reviewBlock}` +
  ...
  // Fix 2: every other Intent-phase dispatch that can fail degrades to a note + safe fallback
  // (see triageP above) instead of propagating uncaught through Promise.all([triageP, intentP]) — a
  // very large PR ingesting all its bundle parts at once is exactly the kind of run most likely to
  // hit this. `null` is the shape every downstream consumer already treats as "no intent": intentBrief/
  // synthIntent/criticIntent return the falsy value as-is, screenPacket does `intent ?? {}`, and the
  // one raw-field read (`intent?.openQuestions`) is already optional-chained. Out of scope for this
  // fix: a new total-bundle-size threshold/fallback for very large PRs — D3 and intent were already
  // whole-diff consumers before bundle parts existed, so this is failure-handling parity, not new
  // architecture.
  .catch((e) => { notes.push(`intent-analyzer failed: ${e.message}`); return null; });
...
const aspects = expandAspects(plan.dimensionAgents, shards, { unsharded: ['D3'], allManifest, allParts, routingEnabled: plan.routing?.enabled ?? true });
...
for (const d of triageExtraDims) aspects.push({ dims: [d], agent: plan.dimensionAgentsAll[d], shardId: 'triage', files: [], manifest: allManifest, count: plan.fileCount ?? 0, parts: allParts });
...
const spawnBatchVerifier = (group, verdictByKey) => {
  ...
  // COST LEVER (context fragments for verifiers): a non-taint group is file-scoped, exactly like a
  // dimension reviewer's aspect, so it gets the same on-demand context fragments Task 1 gave reviewers
  // — the enclosing definition/imports/callers a verifier needs to judge whether a finding's claim
  // actually holds, not just the raw diff hunk. Taint groups are left alone: they already reason
  // cross-file over the FULL diff, and a single file's fragment would be the wrong shape for that.
  const gFragments = isTaint ? [] : contextFragmentsFor(group.files);
  const contextFragBlock = gFragments.length
    ? `The enclosing definitions/imports/callers for the file(s) you're verifying are at ${gFragments.join(', ')} (same fragment naming as the diff slice above) — pull the fragment for a file before ruling on a finding there if the diff slice alone doesn't settle it. If a fragment is absent, that file had no extractable context — fall back to your own Read/Grep rather than retrying.\n\n`
    : '';
  const groupModel = (isTaint || group.findings.some((f) => escDirectSev.has(f.severity))) ? verifyModel : firstModel;
  return agent(
    `${gDiffRead} ${focus}\n\n${contextFragBlock}` +
    // Verifiers are the safety net (a wrongly-refuted finding is a silent false negative, the costly
    // miss this whole pass exists to catch) — give them MORE lookup room than a first-pass reviewer's
    // 4-lookup budget, not less: do not starve them. 7 covers a batch spanning a few files (the diff
    // slice + context fragment above are the primary source; this budget is for whatever they still
    // don't settle) without reopening the whole-diff cost this slicing exists to avoid.
    `Make at most 7 additional lookups (Read, Grep, or read-only Bash — git show/log/diff/blame, rg, grep, sed -n; writes are blocked), only when the diff slice and context fragment above are insufficient to rule on a specific finding; never read outside the changed files' directories except a directly named import.\n\n` +
...
const reviewOnce = (a, pass) => {
  ...
  `The CONTEXT PACK guidance above tells you how to get enclosing definitions, imports, and in-repo callers for a file — follow it. Make at most 4 additional lookups (Read, Grep, or read-only Bash — git show/log/diff/blame, rg, grep, sed -n; writes are blocked), only when the pack is insufficient for a specific suspected finding (name the suspicion in the finding's evidence); never read outside the changed files' directories except a directly named import.\n` +
