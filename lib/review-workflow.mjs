...
const { plan, bundle, diffPath, contextPackPath, diffIndex, history, testSignal, shards, routing, flags, startedAt, prNumber, checkout, sliceIndex, doctrinePaths, contextPackStats, knownFalsePositives } = A;
// build-args strips the flat file list from the embedded plan (it duplicated the per-file paths
// already carried in args.shards, bloating the emitted payload). Rebuild it from shards so the
// plan.files readers below (reviewer + triage packets) keep working unchanged.
if (plan && !Array.isArray(plan.files)) plan.files = (shards ?? []).flatMap((s) => s.files ?? []);
