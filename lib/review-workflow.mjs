...
function expandAspects(dimensionAgents = {}, shards = [], { unsharded = [], allManifest = null, allParts = null } = {}) {
  ...
  const add = (dim, agent, shardId, files, manifest, count, parts, isFullScope) => {
  ...
  for (const [dim, agent] of Object.entries(dimensionAgents)) {
    ...
    for (const s of list) add(dim, agent, s.label, s.files ?? [], s.manifest ?? null, s.count ?? (s.files ?? []).length, s.parts ?? null, false);
  }
  // Dimension-routing pass (#9): narrow a non-full-scope aspect's files to what routedFiles
  // resolves for its dims, now that every dim sharing this (agent, shardId) key is known (a
  // second dim folded into an existing aspect by `add` above must get to influence the file list
  // too, which a single narrow-at-add-time pass could not do). `manifest`/`parts` are nulled only
  // when narrowing actually changed the list — they still point at the FULL shard's manifest/
  // bundle on disk, which no longer matches a narrowed list; scopeFor's existing inline-`files`
  // fallback then handles the narrowed, unbundled list correctly.
...
const packBlock = contextDir
  ? `Per-file CONTEXT FRAGMENTS (enclosing definitions of the changed code, import blocks, and in-repo callers of the changed exports — one fragment per file) are on disk at ${contextDir}/<slice-name>, named with the SAME slice-name convention as your diff slice (lib/trim-diff.mjs sliceName) — if you already know a file's diff-slice name, you already know its context-fragment name. Do NOT read them all upfront: pull ONLY the fragment for a file you suspect has a finding, before finalizing that finding, using your existing Read/Bash lookup budget below (no separate budget for this).\n\n`
  : contextPackPath
...
const spawnBatchVerifier = (group, verdictByKey) => {
  ...
  const contextFragBlock = gFragments.length
    ? `The enclosing definitions/imports/callers for the file(s) you're verifying are at ${gFragments.join(', ')} (same fragment naming as the diff slice above) — pull the fragment for a file before ruling on a finding there if the diff slice alone doesn't settle it.\n\n`
    : '';
