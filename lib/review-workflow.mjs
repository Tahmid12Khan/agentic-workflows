...
const reviewed = await pipeline(
  aspects,
  // stage 1: review one aspect. Prompt ordering is deliberate for prompt-cache reuse: the
  // shared, byte-identical blocks (intent brief + project rules) lead so they cache across every
  // aspect of the same reviewer agent; the per-aspect bits (shard diff, then the dim + file list)
  // trail so they never poison that prefix. (Same intra-agent prefix-first trick the verify pass uses
  // across same-scope findings — it does NOT cache across the intent/review/verify boundary, since
  // those are distinct agents with distinct system prompts.)
  (a) => agent(
    `${packBlock}` +
