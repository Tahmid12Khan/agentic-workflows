...
// Applied ONLY to the Intent-phase agent (intent-analyzer): it does not review a
...
// --- inlined diff-scope filter (canonical + tested: lib/trim-diff.mjs buildDiffIndex,
// lib/review-orchestration.mjs inDiffScope/partitionByScope) ---
...
const { plan, bundle, diff, contextPack, history, testSignal, shards, routing, flags, startedAt, prNumber, checkout } = A;
// Shared context pack (context-pack.mjs, built once in /review step 3): the enclosing definitions
// of changed code, import blocks, and in-repo callers of changed exports. Prepended to every
// reviewer packet so a reviewer reads it FIRST and needs at most a few extra Read/Grep calls
// (S2 of plan.md). It is byte-identical across every aspect of the same reviewer agent, so it
// LEADS the packet and prompt-caches across them. '' when the pre-step was skipped → reviewers
// fall back to their own Read/Grep. The caller section rides in the pack; the correctness
// reviewer is its primary consumer (cross-file consequence, formalized in S6.2).
const packBlock = contextPack ? `${contextPack}\n\n` : '';
...
const intent = await agent(
  ...
  { agentType: pluginAgent('intent-analyzer'), phase: 'Intent', schema: INTENT_SCHEMA },
...
// Verify with the same cheap→strong escalation (unbounded gate=null — the gap set is already small).
// Returns the verified-fresh findings for the caller to concat. Advisory — proposes, never edits.
async function reDispatchGaps(gaps, tag) {
  ...
  const reDispatched = await parallel(gaps.map((g) => () =>
    agent(
      ...
      `Acceptance criteria: ${JSON.stringify(brief)}. Project rules: ${JSON.stringify(plan.projectRules)}. Diff:\n${diff}`,
  ...
  return parallel(fresh.map((f) => () => (plan.runVerify ? verifyWithEscalation(f, null) : Promise.resolve(f))));
}
...
// poison that prefix. (Same intra-agent prefix-first trick the verify pass uses across same-scope
// findings — it does NOT cache across the intent/review/verify boundary, since those are distinct
// agents with distinct system prompts.) `pass` (1|2) tags the S7.1 double-run passes in the label; 0
// = a single (non-exhaustive) run and adds no suffix. Returns raw {findings,strengths}, null on fail.
const reviewOnce = (a, pass) => agent(
  `${packBlock}` +
  ...
  { agentType: pluginAgent(a.agent), model: plan.models?.[a.dim], label: `review:${a.dim}:${a.shardId}${pass ? `#${pass}` : ''}`, phase: 'Review', schema: FINDINGS_SCHEMA },
).then((r) => { bump(a.agent); return { findings: r?.findings ?? [], strengths: r?.strengths ?? [] }; })
  .catch((e) => { notes.push(`review ${a.dim}/${a.shardId}${pass ? ` pass ${pass}` : ''} failed: ${e.message}`); return null; });

// S7.1: in exhaustive mode the correctness + vuln reviewers (the highest cost-of-miss) run TWICE — two
// INDEPENDENT passes unioned + deduped by findingKey (dedupeFindings) BEFORE Verify, so a finding both
// passes agree on is verified once, not twice. This is a real decorrelation lever; with the verifier's
// cheap→strong model_escalate it is the ONLY one that survived honest scoping (findings-withheld is
// already the default and reverse-shards is inert). Both passes bump the run count, so "Agents &
// coverage" shows the doubled dispatch. Intentionally uncached and intentionally spends more — an
// exhaustive-only trade. A failed pass degrades to nothing (never crashes the run).
...
// ---------------------------------------------------------------- Tier B: completeness SCREEN (high tier, cheap)
// S6.1 of plan.md. A CHEAP false-negative screen for the HIGH tier (haiku, no diff), run only when
// the full exhaustive critic above did NOT (they never both run). It sees coverage metadata ONLY —
// which dimensions ran, the finding titles, and the raw intent-analyzer (harvester) output — but NO
// diff, so it flags dimension/criterion COVERAGE gaps and CANNOT claim untraced-taint. A gap naming
// an unrun dimension re-dispatches at most 2 targeted reviewers (not 6), whose new findings re-enter
// Verify. Advisory — proposes, never edits.
if (plan.discovery?.completenessScreen) {
  ...
  allFindings = allFindings.concat(await reDispatchGaps(selectGaps(screen?.gaps, 2), 'completeness-screen'));
}
