...
const intentDiffRead = intentParts.length
  ? `Your diff coverage is bundled into ${intentParts.length} part(s): ${intentParts.join(', ')} — Read ALL of them now, `
    + `in one batch of parallel Read calls (parallel Reads in one turn cost one turn, not one turn per file); `
    + `each part concatenates several files' diff-slice content, with a "=== FILE: <path> ===" header marking `
    + `where one file's hunks end and the next begins.\n\n`
    + `Reading all parts gives you complete diff coverage of every changed file — cap any FURTHER Read/Grep/Bash calls at 3, reserved for a specific sibling file you need for context.`
