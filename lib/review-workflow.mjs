...
function expandAspects(dimensionAgents = {}, shards = [], { unsharded = [], allManifest = null } = {}) {
  ...
  const byKey = new Map();
  const order = [];
  const add = (dim, agent, shardId, files, manifest, count) => {
    const key = `${agent} ${shardId}`;
    let a = byKey.get(key);
    if (!a) { a = { dims: [], agent, shardId, files, manifest, count }; byKey.set(key, a); order.push(a); }
    if (!a.dims.includes(dim)) a.dims.push(dim);
  };
  for (const [dim, agent] of Object.entries(dimensionAgents)) {
    if (!agent) continue;
    if (single.has(dim)) { add(dim, agent, 'all', whole, allManifest, wholeCount); continue; }
    for (const s of list) add(dim, agent, s.label, s.files ?? [], s.manifest ?? null, s.count ?? (s.files ?? []).length);
  }
  return order;
}
...
const { plan, bundle, diffPath, contextPackPath, diffIndex, historyPath, testSignal, shards, routing, flags, startedAt, prNumber, checkout, diffRanges, sliceDir, doctrinePaths, contextPackStats, knownFalsePositives, allManifest, buildNotes } = A;
...
const packBlock = contextPackPath
  ...
  : '';
// WS1 review doctrine (args.doctrinePaths = {agent: [absolute .md paths]}, resolved in build-args.mjs
// from lib/doctrine.mjs, tier >= standard only): tell a reviewer to Read its doctrine fragments first.
// Keyed by AGENT so it is byte-identical across that agent's aspects/shards → stays in the cacheable
// prefix. '' for an agent with no mapped doctrine or a trivial/low tier ({} doctrinePaths).
const doctrineFor = (agent) => {
  const files = (doctrinePaths ?? {})[agent] ?? [];
  if (!files.length) return '';
  return `Before reviewing, Read and apply these review-DOCTRINE notes — they refine how you rank, phrase, and structure findings (advisory guidance, not the finding contract): ${files.join(', ')}.\n\n`;
};
...
const aspects = expandAspects(plan.dimensionAgents, shards, { unsharded: ['D3'], allManifest });
