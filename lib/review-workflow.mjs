...
const { plan, bundle, diffPath, contextPackPath, diffIndex, history, testSignal, shards, routing, flags, startedAt, prNumber, checkout } = A;
...
const spawnBatchVerifier = (group, verdictByKey) => {
  ...
  const focus = group.verifier === 'taint-verifier'
...
async function reDispatchGaps(gaps, tag) {
  ...
  const reDispatched = await parallel(gaps.map((g) => () =>
    agent(
      ...
      `Acceptance criteria: ${JSON.stringify(brief)}. Project rules: ${JSON.stringify(plan.projectRules)}. ${diffRead}`,
