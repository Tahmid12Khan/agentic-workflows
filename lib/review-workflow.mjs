...
// Diff FIRST so the (noise-stripped) intent diff is a byte-identical prefix that prompt-caches with
// the verify/review passes that share it. The prompt enforces STAGED reasoning (criteria/groups
// BEFORE assumptions/questions) so merging the two agents does not collapse the producer→consumer
