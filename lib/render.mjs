...
export function renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, learnings, coverage, meta, checkout = null, usage = null, outOfDiff = [], testSignal = null }) {
  ...
  if (outOfDiff?.length) {
    ...
    // outside the changed lines (a reviewer flagging pre-existing code near the change).
    lines.push(`## Out-of-scope observations (${outOfDiff.length})`, '',
    ...
    for (const f of outOfDiff) {
      const vtag = (f.verify && f.verify.passes > 1) ? `verified ×${f.verify.passes}` : 'trusted';
      const tags = [f.dimension, f.severity, vtag].filter(Boolean).join(' · ');
