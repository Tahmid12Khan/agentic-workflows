...
function pipelineRows(plan) {
  ...
  // Batched verification runs every group on a single model (verifyModel, default opus): when the
  // agent count is already hard-capped there is no cheap→strong split to reflect.
  ...
  return [
    ...
    mk('finding-verifier', vmodel, 'batched adversarial refute of unsure findings + reverify guard', !!plan.runVerify, plan.runVerify ? `unsure findings grouped by (lens, file) into ≤${budget} batched verifier(s), then a +1 reverify guard hunts false negatives` : `skipped — verification runs at low+ (tier: ${tier})`),
...
export function renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, learnings, coverage, meta, checkout = null, usage = null, outOfDiff = [], testSignal = null }) {
  ...
  for (const sev of SEV_ORDER) {
    ...
    for (const f of group) {
      ...
      const tags = [f.dimension, f.isNew ? 'new' : null, f.recurring ? 'recurring' : null, vtag].filter(Boolean).join(' · ');
  ...
  if (outOfDiff?.length) {
    lines.push(`## Out-of-scope observations (unverified) (${outOfDiff.length})`, '',
      '_Anchored outside the changed lines — advisory only, excluded from the verdict and `--comment`._', '');
    for (const f of outOfDiff) {
      const tags = [f.dimension, f.severity].filter(Boolean).join(' · ');
      lines.push(`- **${f.title}**${tags ? ` (${tags})` : ''} — \`${f.file}${f.line ? ':' + f.line : ''}\``);
      if (f.evidence) lines.push(`  - evidence: ${f.evidence}`);
      if (f.fix) lines.push(`  - fix: ${f.fix}`);
    }
    lines.push('');
  ...
  const vd = renderVerdict(findings ?? [], { block_on: ['critical'], warn_on: ['high'] }, tier);
...
export function renderHtml(data) {
  const { findings = [], criteria = [], tier = 'standard', needsHuman = [], skipped = [], strengths = [], summary = '', summaryPoints = [], context = {}, verify = {}, coverage = null, meta = null, checkout = null, usage = null, outOfDiff = [], testSignal = null, gate = { block_on: ['critical'], warn_on: ['high'] } } = data;
  ...
  const sevBlock = SEV_ORDER.map(sev => {
    ...
    return `<section class="grp"><h3 class="sev sev-${sev}">${cap(sev)} <span class="n">${g.length}</span></h3>` +
      g.map(f => `<article class="finding">
        ...
        <div class="f-meta">${esc(f.dimension || '')}${f.isNew ? ' · <b class="new">new</b>' : ''}${f.recurring ? ' · <b class="rec">recurring</b>' : ''}${(f.verify && f.verify.passes > 1) ? ` · <b class="ver">verified ×${f.verify.passes}</b> (${f.verify.real}✓/${f.verify.refuted}✗)` : ' · trusted'} · conf ${f.confidence ?? '—'}</div>
  ...
  // and excluded from the verdict/gate/comments (S1.1 of plan.md).
  const outDiffBlock = outOfDiff.length ? `<section class="grp"><p class="hint">Anchored outside the changed lines — advisory only, excluded from the verdict and <code>--comment</code>.</p>` +
    outOfDiff.map(f => `<article class="finding oos">
      <div class="f-head"><span class="f-title">${esc(f.title)}</span><span class="f-loc">${esc(f.file)}${f.line ? ':' + f.line : ''}</span></div>
...
.info-row{display:flex;justify-content:space-between;align-items:stretch;gap:24px;flex-wrap:wrap;margin:16px 0 28px}
.usage{display:flex;flex-direction:column;justify-content:space-between;padding:11px 15px;border:1px solid var(--line);border-radius:9px;background:var(--panel);font-family:var(--mono);font-size:12.5px}
...
.prinfo{font-family:var(--mono);font-size:12.5px;text-align:left;display:flex;flex-direction:column;gap:12px;margin-left:auto;max-width:60%;padding:11px 15px;border:1px solid var(--line);border-radius:9px;background:var(--panel)}
...
@media(max-width:600px){.info-row{gap:14px}.prinfo{margin-left:0;max-width:100%}}
...
.f-meta{font-family:var(--mono);font-size:13px;color:var(--muted);margin:7px 0}.rec{color:var(--accent)}.f-meta .new{color:#4FB8A8}
...
${outDiffBlock ? `<h2>Out-of-scope observations (unverified)</h2>${outDiffBlock}` : ''}
