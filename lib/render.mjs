...
export function renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, learnings, coverage, meta, checkout = null, usage = null, outOfDiff = [], testSignal = null, processAdvisories = [], gate = { block_on: ['critical'], warn_on: ['important'] }, effort = null, reportThreshold = null, round = null, nitRounds = null, contextPackStats = null }) {
  ...
  if (stillOpen.length) {
    ...
    for (const f of stillOpen) lines.push(`- **${f.title}** — \`${f.file}:${f.line}\``);
    lines.push('');
  }
  ...
  if (uncertain.length) {
    ...
    for (const f of uncertain) {
      ...
      lines.push(`- **${f.title}**${tags ? ` (${tags})` : ''} — \`${f.file}:${f.line}\` _(conf ${f.confidence})_`);
      if (f.evidence) lines.push(`  - evidence: ${f.evidence}`);
      if (f.fix) lines.push(`  - fix: ${f.fix}`);
    }
    lines.push('');
...
export function renderHtml(data) {
  ...
  const sevBlock = SEV_ORDER.map(sev => {
    ...
    return `<section class="grp"><h3 class="sev sev-${sev}">${cap(sev)} <span class="n">${g.length}</span></h3>` +
      g.map(f => `<article class="finding">
        ...
        <div class="f-meta">${esc(f.dimension || '')}${f.isNew ? ' · <b class="new">new</b>' : ''}${f.recurring ? ' · <b class="rec">recurring</b>' : ''}${f.persisting ? ' · <b class="rec">persisting</b>' : ''}${notPostableTag(f) ? ` · <b class="rec">${esc(notPostableTag(f))}</b>` : ''}${(f.verify && f.verify.passes > 1) ? ` · <b class="ver">verified ×${f.verify.passes}</b> (${f.verify.real}✓/${f.verify.refuted}✗)` : ' · trusted'} · conf ${f.confidence ?? '—'}</div>
  ...
  const stillOpenBlock = stillOpenFindings.length ? `<section class="grp"><p class="hint">Present in this review and the last — not re-posted as a new comment.</p>` +
    stillOpenFindings.map(f => `<article class="finding"><div class="f-head"><span class="f-title">${esc(f.title)}</span><span class="f-loc">${esc(f.file)}:${f.line ?? '?'}</span></div></article>`).join('') + `</section>` : '';
  // WS3: fresh findings the convergence round / nit cap held back from posting — informational,
