...
//
// `filesFunneled` (#10, optional): when the mega-PR funnel ran FIRST, `filesCapped.total` is the
// funnel's already-reduced candidate count (e.g. 48), not the change's true original file count
// (e.g. 260) — stating only `filesCapped.total` here would be internally inconsistent with the
// funnel WARN rendered just above it, and would understate the PR's real size to a reader who only
// reads this one line. When `filesFunneled` is present, name the TRUE original count explicitly
// (`filesFunneled.totalFiles`) and label `filesCapped.total` for what it actually is.
export function filesCappedWarning(filesCapped, filesFunneled = null) {
  if (!filesCapped || !filesCapped.dropped) return null;
  const { reviewed, total, dropped, max } = filesCapped;
  if (filesFunneled && filesFunneled.mechanicalClusters) {
    return `> ⚠️ **WARN — file limit exceeded (max ${max}).** This change touches **${filesFunneled.totalFiles}** files in total (the mega-PR funnel above reduced that to **${total}** candidates); only the **${reviewed}** highest-risk of those candidates were reviewed and **${dropped}** were not. Issues in the unreviewed files may be missed. Raise \`large_diff.max_review_files\`, split the PR, or review the rest separately.`;
  }
...
// Mega-PR funnel WARN (#10, shared by the markdown + HTML reports): the diff exceeded
// mega_pr.threshold, so mechanical-looking clusters (boilerplate-shaped groups of files) were
// sampled instead of every file getting an individual review. Cluster-level counts only — never a
// per-file skip list — mirroring how filesCappedWarning states an aggregate, not every dropped
// path. null when the funnel never engaged (the overwhelmingly common case).
export function filesFunneledWarning(filesFunneled) {
  if (!filesFunneled || !filesFunneled.mechanicalClusters) return null;
  const { threshold, hot, mechanicalClusters, mechanicalTotal, sampled, skippedTotal, clusters = [] } = filesFunneled;
  const detail = clusters.map((c) => `${c.label} (${c.sampled}/${c.total} reviewed)`).join(', ');
  return `> ⚠️ **WARN — mega-PR funnel engaged (> ${threshold} files).** **${hot}** file(s) reviewed in full; **${mechanicalClusters}** mechanical cluster(s) totaling **${mechanicalTotal}** file(s) were sampled down to **${sampled}** — **${skippedTotal}** not individually reviewed${detail ? ` (${detail})` : ''}. This is a heuristic (directory+extension grouping with low churn variance), not a diff-similarity proof — spot-check the skipped files if they matter.`;
}
...
export function renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, learnings, coverage, meta, checkout = null, usage = null, outOfDiff = [], testSignal = null, processAdvisories = [], gate = { block_on: ['critical'], warn_on: ['important'] }, effort = null, reportThreshold = null, round = null, nitRounds = null, contextPackStats = null, filesCapped = null, filesFunneled = null }) {
  ...
  // Files-funneled WARN (#10): a mega-PR sampled mechanical clusters instead of reviewing every file.
  // Files-capped WARN: the change exceeded max_review_files, so some files were NOT reviewed. Both are
  // loud and near the top — findings on the skipped/dropped files are absent, not clean. Advisory only.
  const ffw = filesFunneledWarning(filesFunneled);
  if (ffw) lines.push(ffw, '');
  const fcw = filesCappedWarning(filesCapped, filesFunneled);
...
${filesFunneled && filesFunneled.mechanicalClusters ? `<div class="cap-warn" style="background:${vColor === '#E23E4E' ? '#E23E4E' : '#F0A92B'};color:#111;padding:10px 14px;border-radius:8px;margin:10px 0;font-weight:600">⚠️ Mega-PR funnel engaged (&gt; ${filesFunneled.threshold} files): ${filesFunneled.hot} file(s) reviewed in full; ${filesFunneled.mechanicalClusters} mechanical cluster(s) totaling ${filesFunneled.mechanicalTotal} file(s) sampled down to ${filesFunneled.sampled} — ${filesFunneled.skippedTotal} not individually reviewed. Heuristic grouping, not a diff-similarity proof.</div>` : ''}
${filesCapped && filesCapped.dropped ? `<div class="cap-warn" style="background:${vColor === '#E23E4E' ? '#E23E4E' : '#F0A92B'};color:#111;padding:10px 14px;border-radius:8px;margin:10px 0;font-weight:600">⚠️ File limit exceeded (max ${filesCapped.max}): ${filesFunneled && filesFunneled.mechanicalClusters ? `${filesFunneled.totalFiles} files changed in total (mega-PR funnel above reduced that to ${filesCapped.total} candidates)` : `${filesCapped.total} files changed`}, only the ${filesCapped.reviewed} highest-risk reviewed — ${filesCapped.dropped} not reviewed. Issues in the unreviewed files may be missed.</div>` : ''}
