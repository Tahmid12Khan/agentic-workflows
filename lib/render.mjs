...
export function renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, context, verify, learnings, coverage }) {
...
export function renderHtml(data) {
  const { findings = [], criteria = [], tier = 'standard', needsHuman = [], skipped = [], strengths = [], summary = '', context = {}, verify = {}, coverage = null, gate = { block_on: ['critical'], warn_on: ['high'] } } = data;
  const kept = findings.filter(f => (f.confidence ?? 100) >= MIN_CONFIDENCE);
...
.cov-sum{font-family:var(--mono);font-size:12px;color:var(--text);margin:0 0 14px}
.cov-grp{margin:0 0 16px}.cov-h{font-family:var(--mono);font-size:11px;letter-spacing:.1em;text-transform:uppercase;margin:0 0 6px}
.cov-h.ran{color:#8FBF5A}.cov-h.off{color:var(--muted)}
.cov-row{display:grid;grid-template-columns:200px 110px 1fr;gap:10px;align-items:baseline;padding:5px 0;border-top:1px solid var(--line);font-size:13px}
.cov-name{font-family:var(--mono);font-size:12px;color:var(--text);word-break:break-all}.cov-meta{font-family:var(--mono);font-size:11px;color:var(--accent)}.cov-why{color:var(--muted)}
