...
export function buildCommentBody(f) {
  ...
  lines.push('', `_advisory · confidence ${f.confidence ?? '—'}${f.verify ? ` · verified ×${f.verify.passes}` : ''} · agentic-code-review_`);
  return lines.join('\n');
}
...
export function buildSummaryComment({ verdict, tier, findings = [], needsHuman = [] }) {
  ...
  return [`### Code review — **${verdict}** (tier: ${tier})`, '', counts + '.',
    ...
    '\n_advisory · never edits code · agentic-code-review_'].join('\n');
}
