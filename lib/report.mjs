...
export async function generateReport(data = {}, { gateMode = false, baseDir = '.adverserial-code-review' } = {}) {
  ...
  try {
    ...
    const md = renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, coverage, meta, checkout, usage, learnings: { applied: learningNote } });
    writeFileSync(join(outDir, 'review.md'), md);
    const html = renderHtml({ findings, criteria, tier, gate, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, coverage, meta, checkout, usage });
