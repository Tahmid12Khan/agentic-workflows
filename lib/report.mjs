...
//     context, verify, learnings, learningStore, range, startedAt, prNumber, plan, agentRuns }
// By default writes into a per-run folder:
//   .adverserial-code-review/review-{counter}[-pr-{n}]-{YYYY-MM-DD}/review.{md,html}
...
const md = renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, context, verify, coverage, meta, learnings: { applied: learningNote } });
writeFileSync(outPath, md);

const html = renderHtml({ findings, criteria, tier, gate, needsHuman, skipped, strengths, summary, context, verify, coverage, meta });
