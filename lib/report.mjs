...
export async function generateReport(data = {}, { gateMode = false, baseDir = defaultAcrDir() } = {}) {
  ...
  try {
    ...
    const md = renderReport({ findings, criteria, tier, gate, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, coverage, meta, checkout, usage, outOfDiff, testSignal, processAdvisories, learnings: { applied: learningNote }, effort, reportThreshold, round, nitRounds, contextPackStats, filesCapped });
    writeFileSync(join(outDir, 'review.md'), md);
    const html = renderHtml({ findings, criteria, tier, gate, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, coverage, meta, checkout, usage, outOfDiff, testSignal, processAdvisories, effort, reportThreshold, round, nitRounds, contextPackStats, filesCapped });
