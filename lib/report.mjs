...
import { loadLearnings, saveLearnings, recordRun, applyLearnings, dedupAgainstPrevious, loadLastReview, buildLastReview, saveLastReview } from './memory.mjs';
...
export async function generateReport(data = {}, { gateMode = false, baseDir = '.adverserial-code-review' } = {}) {
  ...
  // Incremental review (S9): under --incremental, tag each surviving finding new-vs-carried-over
  // against the PREVIOUS run's last-review.json (read before we overwrite it below). Best-effort — a
  // missing/corrupt state just means "all new". `incremental` flows from the Workflow (flags.incremental).
  const incremental = data.incremental === true;
  const lastReviewPath = join(baseDir, 'last-review.json');
  if (incremental) {
    try {
      const prev = loadLastReview(lastReviewPath);
      if (prev) findings = dedupAgainstPrevious(findings, prev.findings ?? []);
    } catch (e) { notes.push(`incremental dedup skipped: ${e.message}`); }
  }
  ...
  try {
    ...
    const md = renderReport({ findings, criteria, tier, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, coverage, meta, checkout, usage, outOfDiff, testSignal, learnings: { applied: learningNote } });
    writeFileSync(join(outDir, 'review.md'), md);
    const html = renderHtml({ findings, criteria, tier, gate, needsHuman, skipped, strengths, summary, summaryPoints, context, verify, coverage, meta, checkout, usage, outOfDiff, testSignal });
  ...
  if (data.plan?.head) {
    try {
      ...
      saveLastReview(lastReviewPath, buildLastReview({ base: data.plan.base, head: data.plan.head, range: data.range ?? data.plan.range ?? null, findings }));
