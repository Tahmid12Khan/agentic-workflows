#!/usr/bin/env node
// CLI + library: tally the token usage + USD cost of ONE review run from the
// Claude Code session transcripts, so the report can show what the review cost.
//
// A review fans out across many subagents; the orchestrator turns land in the
// session's main transcript (`<project>/<sessionId>.jsonl`) and every
// reviewer/verifier turn lands UNDER `<project>/<sessionId>/subagents/` — either
// directly (`agent-*.jsonl`) or, for a Workflow-dispatched review, nested one or
// more levels deeper (`subagents/workflows/wf_*/agent-*.jsonl`). We therefore walk
// the whole `subagents` subtree, not just its direct children — the earlier
// one-level scan silently missed all reviewer cost on Workflow runs. Every line
// carries a per-message `usage` block (input / output / cache-read / cache-write
// tokens) and the `model` that ran. We scope to THIS session and a time window
// (the review's start) and sum them — "this review run only", as opposed to the
// whole session or all of history. The result also carries a per-(scope,model)
// breakdown and an aggregate cache-hit%.
//
// Cost is derived from a per-model-family price table (USD per million tokens),
// overridable via `.adversarial-code-review/config.json` → `usage.pricing`.
// Best-effort: a missing transcript dir or unparseable line degrades to an empty
// tally (returns null), never throws — the report renders fine without it.
//
// Usage: node usage.mjs [--since <ISO>] [--session <id>] [--cwd <dir>] [--home <dir>]
// Prints { usage: {...} | null } to stdout.
import { readFileSync, readdirSync, existsSync } from 'node:fs';
import { join } from 'node:path';
import { homedir } from 'node:os';

// Per-model-family pricing, USD per MILLION tokens. cacheWrite5m = 1.25× input,
// cacheWrite1h = 2× input, cacheRead = 0.1× input (Anthropic prompt-caching
// economics). Claude Code writes 1h cache entries, so the 5m/1h split matters —
// see costOf. All Opus 4.x share one price; family match is by substring so a
// point release (4.6/4.7/4.8) needs no table change.
export const DEFAULT_PRICES = {
  opus: { input: 5, output: 25, cacheWrite5m: 6.25, cacheWrite1h: 10, cacheRead: 0.5 },
  sonnet: { input: 3, output: 15, cacheWrite5m: 3.75, cacheWrite1h: 6, cacheRead: 0.3 },
  haiku: { input: 1, output: 5, cacheWrite5m: 1.25, cacheWrite1h: 2, cacheRead: 0.1 },
  fable: { input: 10, output: 50, cacheWrite5m: 12.5, cacheWrite1h: 20, cacheRead: 1.0 },
};

// Merge config overrides over the defaults, per family + per field (so a config
// can correct just opus.input without restating the whole table).
export function resolvePrices(config = {}) {
  const over = config?.usage?.pricing ?? {};
  const out = {};
  for (const fam of Object.keys(DEFAULT_PRICES)) out[fam] = { ...DEFAULT_PRICES[fam], ...(over[fam] ?? {}) };
  return out;
}

// A model id → its family name. Unknown ids fall back to opus: the orchestrator
// runs on opus and an unrecognized string is most likely an opus variant, so we
// over- rather than under-state the cost of a miss.
export function familyOf(model) {
  const id = String(model ?? '').toLowerCase();
  if (id.includes('haiku')) return 'haiku';
  if (id.includes('sonnet')) return 'sonnet';
  if (id.includes('fable') || id.includes('mythos')) return 'fable';
  return 'opus';               // opus + any unknown
}

// A model id → its family rate (via familyOf).
export function priceFor(model, prices = DEFAULT_PRICES) {
  return prices[familyOf(model)];
}

// USD cost of ONE usage block. Cache writes are priced by their 5m/1h split when
// the transcript carries it (usage.cache_creation), else the whole creation count
// is treated as 5m — the cheaper rate, so an absent split never overstates cost.
export function costOf(usage = {}, model, prices = DEFAULT_PRICES) {
  const p = priceFor(model, prices);
  const split = usage.cache_creation && typeof usage.cache_creation === 'object' ? usage.cache_creation : null;
  const w5 = split ? (split.ephemeral_5m_input_tokens ?? 0) : (usage.cache_creation_input_tokens ?? 0);
  const w1 = split ? (split.ephemeral_1h_input_tokens ?? 0) : 0;
  const dollars =
    (usage.input_tokens ?? 0) * p.input +
    (usage.output_tokens ?? 0) * p.output +
    (usage.cache_read_input_tokens ?? 0) * p.cacheRead +
    w5 * p.cacheWrite5m +
    w1 * p.cacheWrite1h;
  return dollars / 1_000_000;
}

const emptyTally = () => ({ inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, costUsd: 0, messages: 0 });

// A line is in-window when no window is set, or its timestamp is >= sinceMs. A line
// with no/invalid timestamp is dropped when a window is set — it can't be attributed
// to this run.
function inWindow(o, sinceMs) {
  if (sinceMs == null) return true;
  const ts = Date.parse(o?.timestamp);
  return !(Number.isNaN(ts) || ts < sinceMs);
}

// Add one usage block into a tally accumulator (mutates + returns acc).
function foldUsage(acc, u, model, prices) {
  acc.inputTokens += u.input_tokens ?? 0;
  acc.outputTokens += u.output_tokens ?? 0;
  acc.cacheReadTokens += u.cache_read_input_tokens ?? 0;
  acc.cacheWriteTokens += u.cache_creation_input_tokens ?? 0;
  acc.costUsd += costOf(u, model, prices);
  acc.messages += 1;
  return acc;
}

// Pure: sum the usage across parsed transcript line objects. Lines without a
// message.usage block (user turns, tool results, meta) or outside the window are
// ignored.
export function tallyLines(lines, { sinceMs = null, prices = DEFAULT_PRICES } = {}) {
  const acc = emptyTally();
  for (const o of lines) {
    if (!inWindow(o, sinceMs)) continue;
    const u = o?.message?.usage;
    if (!u) continue;
    foldUsage(acc, u, o?.message?.model, prices);
  }
  return acc;
}

// Pure: same as tallyLines but split into one tally per model family (opus/sonnet/
// haiku/fable). Used to attribute cost across the reviewer fan-out — the model is
// the only per-message identity the transcripts reliably carry (per-dimension
// attribution would need a dispatch-time manifest the Workflow sandbox cannot write).
export function tallyByFamily(lines, { sinceMs = null, prices = DEFAULT_PRICES } = {}) {
  const out = {};
  for (const o of lines) {
    if (!inWindow(o, sinceMs)) continue;
    const u = o?.message?.usage;
    if (!u) continue;
    const fam = familyOf(o?.message?.model);
    foldUsage(out[fam] ?? (out[fam] = emptyTally()), u, o?.message?.model, prices);
  }
  return out;
}

// Cache-read as a fraction of the tokens that were NOT freshly written (read + input).
// null when there is nothing to divide (no input and no cache read) so the renderer
// can omit the metric rather than show a meaningless 0%.
export function cacheHitPct(t = {}) {
  const denom = (t.cacheReadTokens ?? 0) + (t.inputTokens ?? 0);
  return denom > 0 ? (t.cacheReadTokens ?? 0) / denom : null;
}

function addTally(a, b) {
  a.inputTokens += b.inputTokens; a.outputTokens += b.outputTokens;
  a.cacheReadTokens += b.cacheReadTokens; a.cacheWriteTokens += b.cacheWriteTokens;
  a.costUsd += b.costUsd; a.messages += b.messages;
  return a;
}

// Claude Code stores a session's transcript at projects/<encoded-cwd>/. The cwd
// is encoded by replacing every '/' and '.' with '-'.
export function encodeProjectDir(cwd) {
  return String(cwd).replace(/[/.]/g, '-');
}

// Recursively collect every *.jsonl path under a directory. Best-effort: an
// unreadable dir contributes nothing rather than throwing.
function walkJsonl(dir) {
  const out = [];
  let entries;
  try { entries = readdirSync(dir, { withFileTypes: true }); } catch { return out; }
  for (const e of entries) {
    const p = join(dir, e.name);
    if (e.isDirectory()) out.push(...walkJsonl(p));
    else if (e.isFile() && e.name.endsWith('.jsonl')) out.push(p);
  }
  return out;
}

// The transcript files for ONE review: the session's main transcript plus every
// subagent transcript it spawned. Each is tagged with a `scope` — 'orchestrator'
// for the main/top-level transcript, 'subagents' for anything under a `subagents`
// subtree — so cost can be split orchestrator-vs-fan-out. The subagents subtree is
// walked RECURSIVELY: Workflow reviewer transcripts nest under
// `subagents/workflows/wf_*/`, which the earlier direct-children-only scan missed.
// When the session id is unknown we fall back to every transcript in the project
// dir (the time window still scopes it to the run) — broader, but never empty for
// lack of an id.
export function transcriptFiles({ home, cwd, sessionId }) {
  const dir = join(home, '.claude', 'projects', encodeProjectDir(cwd));
  if (!existsSync(dir)) return [];
  const files = [];
  const addSubagents = (base) => {
    for (const f of walkJsonl(join(base, 'subagents'))) files.push({ file: f, scope: 'subagents' });
  };
  if (sessionId) {
    const main = join(dir, `${sessionId}.jsonl`);
    if (existsSync(main)) files.push({ file: main, scope: 'orchestrator' });
    addSubagents(join(dir, sessionId));
  }
  if (files.length === 0) {                            // fallback: the whole project dir
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      if (e.isFile() && e.name.endsWith('.jsonl')) files.push({ file: join(dir, e.name), scope: 'orchestrator' });
      else if (e.isDirectory()) addSubagents(join(dir, e.name));
    }
  }
  return files;
}

// WS9: `.adverserial-code-review` was a typo; `.adversarial-code-review` is correct. Prefer the new
// name; fall back to the old one only if it's the ONLY one present — supports un-migrated installs
// for one release cycle.
function readConfig() {
  const dir = (existsSync('.adverserial-code-review') && !existsSync('.adversarial-code-review'))
    ? '.adverserial-code-review'
    : '.adversarial-code-review';
  try { return JSON.parse(readFileSync(`${dir}/config.json`, 'utf8')); } catch { return {}; }
}

// Top-level: the usage of this review run, or null when there is nothing to show
// (disabled by config, no transcript dir, or no usage in the window) so the
// renderer can simply omit the panel. Defaults source the session + cwd + home
// from the process so report.mjs can call it with just `{ since }`.
export function computeReviewUsage({
  home = homedir(),
  cwd = process.cwd(),
  sessionId = process.env.CLAUDE_CODE_SESSION_ID || null,
  since = null,
  config = null,
} = {}) {
  const cfg = config ?? readConfig();
  if (cfg?.usage?.enabled === false) return null;
  const prices = resolvePrices(cfg);
  const sinceMs = since ? Date.parse(since) : null;
  const files = transcriptFiles({ home, cwd, sessionId });
  const acc = emptyTally();
  const groups = {};                                   // `${scope}|${family}` → tally
  const opts = { sinceMs: Number.isNaN(sinceMs) ? null : sinceMs, prices };
  for (const { file, scope } of files) {
    let text;
    try { text = readFileSync(file, 'utf8'); } catch { continue; }
    const lines = [];
    for (const ln of text.split('\n')) {
      if (!ln.trim()) continue;
      try { lines.push(JSON.parse(ln)); } catch { /* skip a malformed line, never the whole run */ }
    }
    addTally(acc, tallyLines(lines, opts));
    for (const [fam, t] of Object.entries(tallyByFamily(lines, opts))) {
      const key = `${scope}|${fam}`;
      groups[key] = groups[key] ? addTally(groups[key], t) : t;
    }
  }
  if (acc.messages === 0) return null;
  // Per-(scope, model) rows, most-expensive first. Deterministic tie-break on the
  // key so the ordering never depends on object-insertion / filesystem order.
  const breakdown = Object.entries(groups)
    .map(([key, t]) => { const [scope, model] = key.split('|'); return { scope, model, ...t, cacheHitPct: cacheHitPct(t) }; })
    .sort((a, b) => (b.costUsd - a.costUsd) || `${a.scope}|${a.model}`.localeCompare(`${b.scope}|${b.model}`));
  return { ...acc, cacheHitPct: cacheHitPct(acc), scope: sessionId ? 'session' : 'project', breakdown };
}

// --- thin CLI ---
if (import.meta.url === `file://${process.argv[1]}`) {
  const arg = (name, def) => {
    const i = process.argv.indexOf(name);
    return i >= 0 && process.argv[i + 1] ? process.argv[i + 1] : def;
  };
  const usage = computeReviewUsage({
    since: arg('--since'),
    sessionId: arg('--session', process.env.CLAUDE_CODE_SESSION_ID || null),
    cwd: arg('--cwd', process.cwd()),
    home: arg('--home', homedir()),
  });
  process.stdout.write(JSON.stringify({ usage }, null, 2) + '\n');
}
