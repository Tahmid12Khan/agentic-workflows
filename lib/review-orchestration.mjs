...
export function expandAspects(dimensionAgents = {}, shards = [], { unsharded = [], allManifest = null, allParts = null, routingEnabled = true } = {}) {
...
// The Workflow sandbox receives `args` as a JSON STRING, and the orchestrator has to type that
// string verbatim into the tool call. On a deeply-nested tail it reliably miscounts the closing
// braces: on nhst/braze-connector#261 (2026-08-12) 11 of 15 dispatches carried a complete payload
// with ONE surplus trailing `}` (args.json ends `…origin":null}}}` — headCommit/checkout/root — and
// the model wrote four), killing the run at module top-level before a single agent spawned.
//
// Recovery is deliberately message-INDEPENDENT: V8 says "Unexpected non-whitespace character after
// JSON at position N" while the sandbox's engine says only "JSON Parse error: Unable to parse JSON
// string" — no position at all — so a position-based fix would work in tests and fail in production.
// Instead peel trailing closing-brace/bracket/whitespace characters one at a time and retry, bounded.
// Only that character class is ever discarded, so a TRUNCATED or mid-corrupted payload still fails
// loudly rather than being silently reinterpreted as a smaller review.
// Returns { args, repaired, note } so the caller can surface the repair in the run's notes.
