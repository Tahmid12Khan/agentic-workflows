...
export function buildArgs({ plan, bundle, diffPath, contextPackPath, contextPackStats, diffIndex, diffRanges, sliceDir, doctrinePaths, scrutiny, checks, meta, historyPath, testSignal, knownFalsePositives, shards, allManifest, buildNotes }) {
  const m = meta ?? {};
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    const written = (plan.shards ?? []).map((s, i) => {
      ...
      return { label: s.label, count: (s.files ?? []).length, manifest: path };
    });
