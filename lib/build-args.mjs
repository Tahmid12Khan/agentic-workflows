...
import { buildDiffIndex, splitByFile, NOISE_RE, normPath } from './trim-diff.mjs';
...
// Pure: keep only the entries of a per-file map ({path: X}) whose key is one of `files` (compared on
// normPath both sides, since the map may be keyed by the raw diff path). Used to shrink sliceIndex +
// diffIndex to the reviewed set when the file list was capped, so args.json is bounded by
// max_review_files rather than the full changed-file count.
export function restrictIndexToFiles(index, files) {
  const keep = new Set((files ?? []).map(normPath));
  return Object.fromEntries(Object.entries(index ?? {}).filter(([k]) => keep.has(normPath(k))));
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // sliceIndex + diffIndex above are built from the FULL diff. When plan.filesCapped is set the reviewed
  // set was trimmed (plan.files is the capped list), so restrict both to it — otherwise a huge PR would
  // reintroduce every dropped file's per-file entry and defeat the file-count cap that bounds args.json.
  let diffIndex = buildDiffIndex(diffText);
  if (plan.filesCapped) {
    diffIndex = restrictIndexToFiles(diffIndex, plan.files);
    sliceIndex = restrictIndexToFiles(sliceIndex, plan.files);
  }
