...
//   OPTIONAL context.txt (the shared context pack from context-pack.mjs; attached file→file
//   like diff.txt so the bulky pack never enters the agent's context).
//   Prints the assembled args object as one JSON blob on stdout — the caller redirects it
//   to args.json and reads it back exactly ONCE to hand to the Workflow tool. The diff and the
//   context pack (the bulk of the payload) are read from disk here and never enter agent context.
...
export function buildArgs({ plan, bundle, diff, contextPack, scrutiny, checks, meta, history, testSignal }) {
  const m = meta ?? {};
  return {
    plan,
    bundle,
    diff,
    contextPack: contextPack ?? '',   // shared context pack (context-pack.mjs); '' when absent → reviewers fall back to Read/Grep
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const out = buildArgs({
    ...
    bundle,
    diff,
    contextPack: readText('context.txt'),   // OPTIONAL — '' when context-pack.mjs was skipped
