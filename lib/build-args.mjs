...
export function bundleParts(files, byFile, { maxLines = 1800, maxBytes = 200_000 } = {}) {
  const parts = [];
  let current = '';
  let currentLines = 0;
...
export function buildArgs({ plan, bundle, diffPath, contextPackPath, contextPackStats, diffIndex, diffRanges, sliceDir, doctrinePaths, scrutiny, checks, meta, historyPath, testSignal, knownFalsePositives, shards, allManifest, allParts, buildNotes }) {
  const m = meta ?? {};
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // file. Same try block as the manifests so a single failure mode covers both — a bundle-write
  // failure must not partially populate `parts`/`allParts` on some shards and not others.
  let shardsOut = null;
  ...
  try {
    ...
    const written = (plan.shards ?? []).map((s, i) => {
      ...
      const parts = writeBundle(s.files ?? [], safeLabel);
      return { label: s.label, count: (s.files ?? []).length, manifest: path, parts };
    });
