...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    // Nothing to route (no dims/agents this run) — skip entirely, including the mkdirSync calls
    // below, so a run with no candidate routing work never raises a note over an unrelated fs quirk
    // in a directory this step would otherwise have no reason to touch.
    if (Object.keys(agentDims).length) {
      const manifestDir = resolve(dir, 'manifests');
      const bundleDir = resolve(dir, 'bundles');
      mkdirSync(manifestDir, { recursive: true });
      mkdirSync(bundleDir, { recursive: true });
    ...
    buildNotes.push(`routed manifests not written (${e.message}) — reviewers fall back to full-shard scope`);
  }
