...
export function planReview(signals, config, forceTier, maxTier) {
  ...
  const tier = ft ?? capTier(applyRiskMap(baseTier(signals), signals, config), maxTier);
...
export function exhaustivePlan(tier, config = {}, opts = {}) {
  ...
  // S6.1: a CHEAP completeness SCREEN (haiku, no diff) on the high tier — but only when the full
  // exhaustive critic is NOT already running (exhaustive supersedes it, never both). Opt-out via
  // config.completeness.screen_on_high:false. Scoped to dimension/criterion coverage gaps.
  const screenOnHigh = config.completeness?.screen_on_high ?? true;
  return {
    exhaustive: on,
    completenessCritic: on, // false-negative guard after synthesis (review.md step 8b)
    completenessScreen: !on && tier === 'high' && screenOnHigh, // S6.1 high-tier cheap screen
