...
export function exhaustivePlan(tier, config = {}, opts = {}) {
  ...
  // S6.1: a CHEAP completeness SCREEN (sonnet, no diff) runs on EVERY workflow tier (low/standard/high)
  // — one x1 sonnet coverage-gap pass per review — but only when the full exhaustive critic is NOT
  ...
  return {
    ...
    completenessScreen: !on && screenTiers.has(tier) && screenEnabled, // S6.1 cheap sonnet screen on every workflow tier
    // Gap re-dispatch budget for the screen: spend follows blast radius — low trusts the screen and
    // re-dispatches nothing (sonnet coverage note only), standard ≤1, high ≤2. The full exhaustive
